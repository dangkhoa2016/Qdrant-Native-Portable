...
Các lệnh lifecycle native phân biệt process Qdrant còn sống với REST API đã sẵn sàng. `bash qdrant.sh start` chờ authenticated API readiness ngay cả khi PID file xác định một process còn sống, nhờ đó caller chờ xuyên suốt quá trình load/recovery collection khi cold start cho tới lúc REST endpoint phản hồi. Startup deadline theo wall-clock mặc định là 300 giây và có thể ghi đè cho dataset lớn, ví dụ `QDRANT_START_TIMEOUT_SECONDS=600 bash qdrant.sh start`. Xem [hướng dẫn sử dụng chi tiết](docs/USAGE.vi.md#10-startstoprestart) để hiểu timeout, log và failure semantics.
