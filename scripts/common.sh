...
_runtime_vars=(
    ...
    QDRANT_ENABLE_GRPC QDRANT_JWT_RBAC QDRANT_USER DEMO_COLLECTION
...
qdrant_ready() { require_secrets; local host="$QDRANT_BIND_HOST"; [[ "$host" == "0.0.0.0" ]] && host="127.0.0.1"; curl -fsS --max-time 3 -H "api-key: $QDRANT_API_KEY" "http://${host}:${QDRANT_HTTP_PORT}/collections" >/dev/null; }
